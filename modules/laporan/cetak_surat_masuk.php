...
$data  = $conn->query("SELECT * FROM surat_masuk WHERE DATE_FORMAT(tgl_terima,'%Y-%m')='$bulan' ORDER BY tgl_terima");
...
<head>
    <meta charset="UTF-8">
    <title>Laporan Surat Masuk - PTUN Banjarmasin</title>
    <style>
...
while ($r = $data->fetch_assoc()) {
    $html .= '
            <tr>
                ...
                <td>'.date('d/m/Y', strtotime($r['tgl_terima'])).'</td>
                <td>'.$r['pengirim'].'</td>
                <td>'.$r['perihal'].'</td>
                <td>'.($r['status'] ?? 'Belum Diproses').'</td>
            </tr>';
}
$html .= '
        </tbody>
...
$dompdf->render();
$dompdf->stream('surat_masuk_'.$bulan.'.pdf', ['Attachment' => 1]);
?>