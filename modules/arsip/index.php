<?php
$title = 'Arsip Surat – PTUN Banjarmasin';
...
$totalSql = "SELECT COUNT(*) 
             FROM arsip
             WHERE no_surat LIKE ? OR perihal LIKE ? OR pihak LIKE ?";
...
/* ---------- Ambil data ---------- */
$dataSql = "SELECT id, no_surat, tgl_surat, perihal, pihak, jenis, file_path
            FROM arsip
            WHERE no_surat LIKE ? OR perihal LIKE ? OR pihak LIKE ?
            ORDER BY id DESC
            LIMIT ? OFFSET ?";
$stmt = $db->prepare($dataSql);
$stmt->bind_param('sssii', $like, $like, $like, $show, $offset);
$stmt->execute();
...
<head>
  ...
  <style>
    ...
    .sidebar img{width:100px;margin-bottom:10px;border-radius:50%;}
...
<aside class="sidebar text-center">
  ...
  <nav class="text-start">
    ...
    <a href="index.php" class="active"><i class="bi bi-archive me-2"></i>Arsip</a>
...
<main class="main">
  <div class="d-flex justify-content-between mb-3">
    <h3>Arsip Surat</h3>
    <a href="tambah.php" class="btn btn-primary"><i class="bi bi-plus-circle me-1"></i>Tambah Arsip</a>
  </div>
  ...
  <form method="get" class="row g-2 mb-3">
    ...
    <div class="col-auto">
      <input type="text" name="search" class="form-control" placeholder="Cari arsip..." value="<?= htmlspecialchars($keyword) ?>">
    </div>
    <div class="col-auto">
  ...
  <div class="table-wrapper">
    <table class="table table-hover mb-0 align-middle text-nowrap">
      ...
      <?php if ($result && $result->num_rows): ?>
        <?php $no=$offset+1; while($a=$result->fetch_assoc()): ?>
        <tr>
          <td class="text-center"><?= $no++ ?></td>
          <td><?= htmlspecialchars($a['no_surat']) ?></td>
          <td><?= date('d-m-Y', strtotime($a['tgl_surat'])) ?></td>
          <td><?= htmlspecialchars($a['perihal']) ?></td>
          <td><?= htmlspecialchars($a['pihak']) ?></td>
          <td>
            <span class="badge bg-<?= $a['jenis']=='masuk'?'info':'secondary' ?>">
              <?= ucfirst($a['jenis']) ?>
            </span>
          </td>
          <td class="text-center">
            <a href="download.php?file=<?= urlencode($a['file_path']) ?>" class="btn btn-sm btn-success" title="Download">
              <i class="bi bi-download"></i>
            </a>
            <a href="edit.php?id=<?= $a['id'] ?>" class="btn btn-sm btn-warning" title="Edit">
              <i class="bi bi-pencil"></i>
            </a>
            <a href="hapus.php?id=<?= $a['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Hapus arsip?')" title="Hapus">
