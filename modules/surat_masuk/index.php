<?php
$title = 'Surat Masuk – PTUN Banjarmasin';
...
$totalSql = "SELECT COUNT(*) 
             FROM surat_masuk
             WHERE no_agenda LIKE ? OR no_surat LIKE ? OR perihal LIKE ? OR pengirim LIKE ?";
$stmt = $db->prepare($totalSql);
$stmt->bind_param('ssss', $like, $like, $like, $like); // 4 placeholders → 4 's'
$stmt->execute();
...
/* ---------- Ambil data ---------- */
$dataSql = "SELECT id, no_agenda, no_surat, tgl_surat, tgl_terima,
                   pengirim, perihal, status
            FROM surat_masuk
            WHERE no_agenda LIKE ? OR no_surat LIKE ? OR perihal LIKE ? OR pengirim LIKE ?
            ORDER BY id DESC
            LIMIT ? OFFSET ?";
...
<aside class="sidebar text-center">
  ...
  <nav class="text-start">
    ...
    <a href="index.php" class="active"><i class="bi bi-inbox me-2"></i>Surat Masuk</a>
...
<main class="main">
  ...
  <form method="get" class="row g-2 mb-3">
    ...
    <div class="col-auto">
      <input type="text" name="search" class="form-control" placeholder="Cari..." value="<?= htmlspecialchars($keyword) ?>">
    </div>
    <div class="col-auto">
  ...
  <div class="table-wrapper">
    <table class="table table-hover mb-0">
      <thead class="table-light">
        <tr>
          <th>No</th><th>No Agenda</th><th>No Surat</th><th>Tgl Surat</th>
          <th>Tgl Terima</th><th>Pengirim</th><th>Perihal</th><th>Status</th><th>Aksi</th>
        </tr>
      </thead>
      <tbody>
